...
<div class="modal-m-overlay" id="modal-hapus{{ $d->id_petugas }}">
  ...
  <div class="modal-m-footer"><button class="btn-m btn-outline-m" onclick="closeModal('modal-hapus{{ $d->id_petugas }}')">Batal</button><form method="POST" action="{{ route('administrator.petugas.hapus', $d->id_petugas) }}" style="display:inline">@csrf @method('DELETE')<button type="submit" class="btn-m btn-danger-m">Ya, Hapus</button></form></div></div>
</div>
