...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1rem;margin-bottom:1.75rem" class="fade-up delay-2">
  ...
  <a href="{{ route('administrator.petugas') }}" style="text-decoration:none"><div class="card-m" style="padding:1.5rem;display:flex;align-items:center;gap:.9rem;transition:transform .22s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform='none'"><div style="width:44px;height:44px;border-radius:10px;background:var(--gold-p);border:1px solid var(--gold-ln);display:flex;align-items:center;justify-content:center;font-size:1.2rem;flex-shrink:0">👥</div><div><div style="font-size:.9rem;font-weight:600;color:var(--ink)">Data Petugas</div><div style="font-size:.75rem;color:var(--ink-m)">Kelola akun petugas</div></div></div></a>
  <a href="{{ route('administrator.laporan') }}" style="text-decoration:none"><div class="card-m" style="padding:1.5rem;display:flex;align-items:center;gap:.9rem;transition:transform .22s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform='none'"><div style="width:44px;height:44px;border-radius:10px;background:var(--gold-p);border:1px solid var(--gold-ln);display:flex;align-items:center;justify-content:center;font-size:1.2rem;flex-shrink:0">📊</div><div><div style="font-size:.9rem;font-weight:600;color:var(--ink)">Laporan</div><div style="font-size:.75rem;color:var(--ink-m)">Hasil & statistik lelang</div></div></div></a>
</div>
<div class="card-m fade-up delay-3">
  <div class="card-m-header"><div class="card-m-title"><span>📋</span> Lelang Terbaru</div><a href="{{ route('administrator.laporan') }}" class="btn-m btn-outline-m btn-sm-m">Lihat Semua →</a></div>
    ...
    <table class="table-m">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td><strong>{{ $r->barang->nama_barang }}</strong></td>
          <td>Rp {{ number_format($r->barang->harga_awal) }}</td>
