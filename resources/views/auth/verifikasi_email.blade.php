...
<head>
  ...
  <title>Verifikasi Email — LuxBid</title>
...
<body>
  ...
  <div class="auth-wrap">
    <a href="{{ route('daftar.masyarakat') }}" class="auth-back">
    ...
    <p class="auth-sub">
      ...
      @if(session('verif_email_hint'))
        <strong>{{ session('verif_email_hint') }}</strong>.
      @else
        Anda.
      @endif
      Masukkan kode untuk mengaktifkan akun.
    </p>
    ...
    <div class="card-auth">
      ...
      <form method="POST" action="{{ route('daftar.verifikasi.post') }}">
        @csrf
      ...
      <div style="text-align:center;margin-top:1.25rem;font-size:.82rem;color:var(--text-3)">
        Tidak menerima kode?
        <form method="POST" action="{{ route('daftar.verifikasi.kirimulang') }}" style="display:inline">
          @csrf
