...
<table class="table-m">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>{!! $l->_pemenang ?? '<span style="color:var(--ink-l);font-size:.8rem">Belum ada penawaran</span>' !!}</td>
      <td style="font-weight:700;color:var(--success)">{{ $l->_harga_tertinggi ? 'Rp '.number_format($l->_harga_tertinggi) : '—' }}</td>
      <td style="color:var(--ink-m)">Rp {{ number_format($l->barang->harga_awal) }}</td>
    </tr>
    @empty
