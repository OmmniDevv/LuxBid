...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:1rem;margin-bottom:1.75rem" class="fade-up delay-2">
  ...
  <a href="{{ $isAdmin?route('administrator.petugas'):route('petugas.aktivasi') }}" style="text-decoration:none"><div class="card-m" style="padding:1.5rem;display:flex;align-items:center;gap:.9rem;transition:transform .22s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform='none'"><div style="width:44px;height:44px;border-radius:10px;background:var(--gold-p);border:1px solid var(--gold-ln);display:flex;align-items:center;justify-content:center;font-size:1.2rem;flex-shrink:0"><i class="bi bi-{{ $isAdmin?'people':'lightning-charge' }}"></i></div><div><div style="font-size:.9rem;font-weight:600;color:var(--ink)">{{ $isAdmin?'Data Petugas':'Aktivasi Lelang' }}</div><div style="font-size:.75rem;color:var(--ink-m)">{{ $isAdmin?'Kelola akun petugas':'Buka & tutup lelang' }}</div></div></div></a>
  <a href="{{ route('petugas.laporan') }}" style="text-decoration:none"><div class="card-m" style="padding:1.5rem;display:flex;align-items:center;gap:.9rem;transition:transform .22s" onmouseover="this.style.transform='translateY(-3px)'" onmouseout="this.style.transform='none'"><div style="width:44px;height:44px;border-radius:10px;background:var(--gold-p);border:1px solid var(--gold-ln);display:flex;align-items:center;justify-content:center;font-size:1.2rem;flex-shrink:0"><i class="bi bi-bar-chart"></i></div><div><div style="font-size:.9rem;font-weight:600;color:var(--ink)">Laporan</div><div style="font-size:.75rem;color:var(--ink-m)">Lihat hasil lelang</div></div></div></a>
</div>
<div class="card-m fade-up delay-3">
  <div class="card-m-header"><div class="card-m-title"><i class="bi bi-card-list"></i> Daftar Lelang Terbaru</div><a href="{{ route('petugas.aktivasi') }}" class="btn-m btn-outline-m btn-sm-m">Kelola Semua →</a></div>
