...
<div class="modal-m-overlay" id="modal-hapus{{ $d->id_barang }}">
  ...
  <div class="modal-m-footer"><button class="btn-m btn-outline-m" onclick="closeModal('modal-hapus{{ $d->id_barang }}')">Batal</button><form method="POST" action="{{ route('petugas.barang.hapus', $d->id_barang) }}" style="display:inline">@csrf @method('DELETE')<button type="submit" class="btn-m btn-danger-m">Ya, Hapus</button></form></div></div>
</div>
